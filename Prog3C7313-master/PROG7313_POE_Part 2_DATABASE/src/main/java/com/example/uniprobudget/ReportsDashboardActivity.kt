...
class ReportsDashboardActivity : AppCompatActivity() {
    ...
    private lateinit var budgetChart: BudgetProgressChart

    private lateinit var categoryChart: CategorySpendingChart

    private var currentMinGoal = 0.0
    private var currentMaxGoal = 0.0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val btnLoadDashboard =
                findViewById<MaterialButton>(R.id.btnLoadDashboard)

            etMonth.setText("2026-04")
            rvCategoryTotals.layoutManager =
                LinearLayoutManager(this)

            btnLoadDashboard.setOnClickListener {
                loadDashboard()
            }
    ...
    private fun loadDashboard() {
        ...
        if (!monthYear.matches(Regex("\\d{4}-\\d{2}"))) {
            Toast.makeText(this, "Month must be YYYY-MM format (e.g., 2026-04)", Toast.LENGTH_SHORT)
                .show()
            return
        }
            ...
            try {
                repository.getCategoryReport(startDate, endDate).collect { categories ->
                    ...
                    if (categories.isEmpty()) {
                        ...
                        rvCategoryTotals.adapter =
                            CategoryTotalsAdapter(categories)

                        categoryChart.updateData(
                            categories,
                            currentMinGoal,
                            currentMaxGoal
                        )
    ...
    private fun updateBudgetUI(status: BudgetStatus) {
        ...
        currentMinGoal = status.minGoal
        currentMaxGoal = status.maxGoal
